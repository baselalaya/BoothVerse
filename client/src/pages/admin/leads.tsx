            ...
            <tbody>
              {data.map(l => (
                <tr key={l.id} className="border-t border-white/10">
                  ...
                  <td className="p-2">
                    <select value={l.status} onChange={async (e)=>{ const status=e.target.value; await adminApi('PATCH', `/api/leads/${l.id}`, { status }); setData(prev=>prev.map(x=>x.id===l.id?{...x,status}:x)); }} className="bg-black/40 border border-white/15 rounded px-2 py-1">
                      {['new','in_progress','closed'].map(s=> <option key={s} value={s}>{s}</option>)}
                    </select>
                  </td>
                  <td className="p-2">{new Date(l.created_at).toLocaleString()}</td>
                  <td className="p-2">
                    <button onClick={()=>openLeadLocal(l)} className="rounded bg-white/10 px-2 py-1 hover:bg-white/15">View</button>
                  </td>
                </tr>
              ))}
